...
- **Modular Design**: While our current focus is on *Uniswap v4*, these implementations are built to be agnostic and adaptable to any modular DeFi structure.
...
- **Authority Verification**: The `IdentiFIHook` intercepts the swap call to verify if the actor is authorized via `hookData` by interacting with the local ZK engine in [identifi-core](https://github.com/IdentiFI-Protocol/identifi-core) engine.
